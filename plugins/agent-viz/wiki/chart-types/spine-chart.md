...
**Reach for it when** each row is one total split in two, the split is the message, and the two parts are opposed in kind rather than ordered. Two components and one row per category is the whole of its range.
...
- [diverging-bar-chart.md](diverging-bar-chart.md) — the other form in this group, and the one to use when the row carries one signed number
