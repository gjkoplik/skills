...
Also called a **band chart**. A **100% stacked area**, sometimes called a normalized stacked area, is the same chart with each column rescaled to shares, and it makes a different claim: it shows composition and deletes the total, exactly as the 100% case does on [stacked-bar.md](stacked-bar.md). Keep the two apart; they are not interchangeable.
...
**Two naming problems, both real, both from sources in this wiki.** [Schwabish](../sources/schwabish.md) files **Area** and **Stacked Area** as two separate types in his Time chapter, while [the FT](../sources/ft-visual-vocabulary.md) carries a single **Area chart** gloss whose warning is about "seeing change in components", and components only exist once you stack. So one scheme separates the forms and the other writes one entry that is really about this one. Separately, [Skau & Kosara (2016)](../studies/skau-kosara-2016.md) use the phrase "area chart" for a pie-cue isolation stimulus that is neither form; [area-chart.md](area-chart.md) records that collision because it produces confident miscitations. The name **band chart** is not vouched by any source in this wiki, so treat it as a name in circulation rather than a definition.

## When to reach for it, and when not

**Reach for it when** the total is a real quantity the reader cares about, the components are few, x is continuous and ordered, and what the reader should take away is the shape of the total plus the coarse relative size of the bands. That is the whole of its range.
...
| Only the shares matter | A 100% stacked area, knowing it removes the total. If the total was not the reason you stacked, a set of share lines is simpler |
...
**Three one-slot relationships.** Add series to the transform slot of an [area chart](area-chart.md) and you get this. Replace the continuous x with categories and you get [stacked-bar.md](stacked-bar.md). Let the baseline be computed from the data instead of fixed at zero and you get [streamgraph.md](streamgraph.md).
...
**Whether the components should be ordered by size, by importance, or by convention.** Nobody in this corpus has tested any ordering. The one thing that follows from the evidence rather than from taste is that whichever series matters most belongs on the baseline, and that follows from the measured bar result rather than from a study of orderings.

## The failure mode it invites

**Inviting the reader to track a middle band.** The chart shows every component at once and makes them all look equally readable. Following the orange band across the chart is a set of thickness judgments with no shared reference, which is the one comparison the divided-bar literature measured and found expensive. The remedy is the same one [stacked-bar.md](stacked-bar.md) gives: put the series in question on the baseline, or take it out and draw it as its own line.
