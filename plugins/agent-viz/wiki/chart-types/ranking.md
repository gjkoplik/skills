...
**Would the sentence survive replacing every value with its position?** "Germany is third" is a ranking sentence. "Germany is 3.2 times Poland" is a magnitude sentence wearing an ordering. If the number has to be in the sentence, the question is next door.
...
If a test fails, the question belongs to a different group:
...
| Which is biggest, and in what order? | **This group** |
| Who overtook whom? | **This group**, and see [bump-chart.md](bump-chart.md) |
| Did the order change between two dates? | **This group**, and see [slope-chart.md](slope-chart.md) |
| Two values per item, and the gap between them is the point | **This group**, and see [dumbbell-plot.md](dumbbell-plot.md) |

**The usual form here is a sorted bar chart, and it is indexed under [magnitude](magnitude.md).** That index says the same thing from its side, that magnitude and ranking mostly produce the same object. This group earns a separate index because a handful of forms exist that only make sense once the value has stopped being the message, not because the ordinary case needs a special shape. Where a sorted bar chart answers the question, it is the best-evidenced form in this wiki.

## What this group costs

**The evidence does not follow into this group.** Almost the whole graphical-perception literature measures one task: read a value off a mark and report it as a number. Cleveland & McGill state the scope limit themselves, and it is quoted in [the scope-limit section of channels.md](../concepts/channels.md#what-the-ranking-is-not-about). A group whose entry condition is *the value is not what the reader wants* has, by construction, defined itself out of the task that was measured.
...
`authority-asserted` on both sides.
...
| Form | What carries the order | Defined for |
|---|---|---|
...
| [Dumbbell plot](dumbbell-plot.md) | Two positions per row on one shared axis, with the segment between them drawing the gap | Two values per item, and the levels matter as well as the gap. Where only the gap matters, the differences themselves are the plot |
...
- **The value stays on the chart unless something takes it off.** All but one of the forms above keep it. The [bump chart](bump-chart.md) is the exception, and the discard is the whole trade it offers.
- **The sort and the population.** Both are choices the figure cannot show. A caption line carries them.
- **Comparison of two rank charts with different item sets.** The ranks are not on the same scale, because a rank has no scale independent of the set it came from.

## Justifying the choice

**Defensible, evidence-backed:**

Almost nothing, and the reason is structural rather than an accident of coverage. The one evidence-backed sentence available anywhere near this group is about reading values off position along a common scale, which is the task this group's entry condition rules out. It applies only to the sorted bar:
...
Perturbing each value by its uncertainty and re-sorting changes the figure; where the sentence under the chart stops being true, the sentence was about the sorting rather than about the items. `authority-asserted`, and it follows from the structure rather than from a study.
...
None of them has a study behind it. The sorted [bar chart](bar-chart.md) is where the evidence in this neighborhood lives, and it is indexed under [magnitude](magnitude.md).
