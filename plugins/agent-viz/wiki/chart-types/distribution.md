...
| I have four observations per group | Plot the four observations. A strip plot or univariate scatterplot, not a summary form |
| What shape is this variable, and does it have one mode or two? | **Stay here** |
...
**Most of these forms have a knob, and the knob is an analysis choice.** Bin width for a histogram, bandwidth for a violin. Wilke's chapters 7 and 14 treat both as analysis choices without giving a rule ([wilke-fundamentals.md](../sources/wilke-fundamentals.md)), and [inventory.md](../inventory.md) topic 52 asks you to state them and check the conclusion survives a different one. `authority-asserted`. The box plot's absence of a knob is a deliberate design property, not an accident ([john-tukey.md](../people/john-tukey.md)).
...
| Form | What is actually drawn | Reach for it when |
|---|---|---|
...
| [Violin plot](violin-plot.md) | A mirrored density estimate | Shape matters across groups, or you are encoding a mean and its error and want a symmetric continuous mark |
| [Ridgeline](ridgeline-plot.md) | One density estimate per group, stacked and overlapping | Many groups in a known order have to share one value axis in one figure, and you accept occlusion to get them there. Untested |
...
- **At small n, plot the points.** Two groups arrive there by different routes, one from a 703-article review and one from expert guidance, and neither ran a reader experiment.
- **If you draw an interval, name it and state n.** Cumming et al.'s rules 1 and 2 are the mechanizable pair. Belia et al. is why the label is necessary and not sufficient: 473 published authors, asked to set two means "just significantly different," and "only 22% of respondents set the means so the p value was between .025 and .10." The gap between the CI group's average and the SE group's was 48 units where the correct answers sit 160 apart, and 31.5% used the just-touching rule, which is wrong for both bar types in opposite directions.
- **For an inferential comparison, prefer a symmetric, continuous encoding over a bar with error bars.** Correll & Gleicher, measured: the bar's "false metaphor of containment" made outcomes inside the bar look likelier than equidistant outcomes above it, an effect absent from all three symmetric encodings. Scope: crowdsourced, low-stakes prediction, 240 participants, no real decisions.
- **If rows were dropped, say so.** [Song & Szafir (2019)](../studies/song-szafir-2019-missing-data.md) tested time series rather than distributions, and found that information removal "can significantly degrade perceptions of data quality, and confidence" and can produce incorrect answers when it breaks visual continuity. What transfers is that removal is not a neutral act. What is untested is the case a plotting library hits most often, which is a caller who never noticed rows were dropped; the paper says so itself. The mechanical version is in [checks/matplotlib.md](../checks/matplotlib.md): compare the input row count to the plotted count.
...
There is a second, quieter version specific to this group: the choice of summary does rhetorical work whether or not you intend it. "Showing the SE rather than the SD magnifies the apparent visual differences between groups. This effect is exacerbated when the groups being compared have different sample sizes."
