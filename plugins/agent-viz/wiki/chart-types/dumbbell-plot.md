...
**The form applies where** each category carries exactly two values of the same quantity (two periods, two groups, two conditions) and **both the levels and the gap are part of the message**. Where only one of the two is the message there is another chart in each direction: levels alone are a [bar chart](bar-chart.md) or a [lollipop chart](lollipop-chart.md), and the gap alone is a sorted bar chart of the differences.
...
|---|---|
| Only the size of the gap matters, not where each item sits | A sorted bar chart of the differences, which puts the quantity of interest back on position from a shared zero. See below |
...
| The second mark is an interval around an estimate, not a second measurement | Not this form. Uncertainty drawn and labeled as uncertainty; expert readers misread error bars badly ([belia-2005-ci-misconceptions.md](../studies/belia-2005-ci-misconceptions.md)) |
...
## Where the gap is the whole message
...
The cost is real: it throws the levels away. Two categories with a gap of four might be at 104 and 4004, and the difference chart cannot tell them apart, which is [deviation.md](deviation.md)'s standing trade. The dumbbell is the form for the case where that trade is not acceptable.
...
- [ranking.md](ranking.md) — the group, and the free parameters it carries
