...
**What it is.** A rectangle recursively subdivided so that each sub-rectangle's area is proportional to its value. The standard answer for part-to-whole with hierarchy or with many parts.

**Status.** One directly relevant `primary-read` study, [Heer & Bostock (2010)](../studies/heer-bostock-2010.md), which tested treemap area judgments specifically and produced a counterintuitive result about aspect ratio.

**What it is good for.** Knowing what rectangular area actually costs, and why the squarified layout everyone uses is optimizing for the wrong thing on one axis.

**What it does not settle.** Hierarchy reading, which is the treemap's other job and is untested here.

**Relationships.** [Part-to-whole](part-to-whole.md), Magnitude. An unlinked relationship means that index is not written yet.
...
|---|---|
| Which is biggest, and in what order? | Sorted bar chart. Rank 4 does not support reliable ordering by eye |
...
| How big is this part, precisely? | Bar chart. Area is rank 4 |
...
**Position contributes nothing.** Rectangles float in a subdivided plane with no common baseline and no axis. The treemap gives up rank 1 and rank 2 entirely, which is the price of packing hundreds of parts into a fixed canvas.
...
**Value extraction**, at rank 4. Worse than position, worse than angle. Evidence-backed via Heer & Bostock.

**Squares specifically.** This is the surprising one:
...
**Reading a treemap as a ranking.** Adjacent rectangles of similar size are not reliably orderable by eye at rank 4, and the layout puts large rectangles in a corner, which reads as importance beyond what the areas encode. If the reader's question is "which is biggest," a sorted bar chart answers it at rank 1. `authority-asserted`.
...
- ~~"You can see which of these two is bigger."~~ Not reliably, at rank 4, for similar sizes.
